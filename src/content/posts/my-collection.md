---
title: "My Collection"
date: 2026-02-18
tags: ["coins"]
excerpt: "A quick look at pieces from my ancient coin collection."
cover: "/coins/my-collection-cover.png"
draft: false
---

A snapshot of coins from my collection, including some tetradrachms and staters.

### Collection video

<video controls playsinline preload="metadata" style="max-width:100%; border:1px dashed var(--border); border-radius:6px;">
  <source src="/coins/IMG_8004.mp4" type="video/mp4" />
  <source src="/coins/IMG_8004.MOV" type="video/quicktime" />
  Your browser does not support the video tag.
</video>

### Obverse views (some tetradrachms and staters)

![Coin obverse side details in hand](/coins/my-collection-fronts.png)

### Reverse views (some tetradrachms and staters)

![Coin reverse side details in hand](/coins/my-collection-backs.png)
